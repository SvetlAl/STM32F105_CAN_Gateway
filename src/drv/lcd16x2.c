...
void lcd16x2_init(uint8_t disp_attr)
{
	...
	// Delay power on 
	uint32_t del=LCD16X2_DELAY_POWER_ON;
	while(del--);
	...
	del=LCD16X2_DELAY_INIT;
	while(del--);
	
	// Repeat last command
	lcd16x2_toggle_e();
	// Delay, busy flag can't be checked here
	del=LCD16X2_DELAY_INIT_REP;
	while(del--);
...
static void lcd16x2_toggle_e()
{
	...
	// Pulse length in us
	uint32_t del=LCD16X2_DELAY_ENABLE_PULSE;
	while(del--);
	// EN pin = LOW
...
static uint8_t lcd16x2_read(uint8_t rs)
{
	...
	GPIOA->CRL |=  GPIO_CRL_CNF3_0;  //дискретный вход, третье состояние
	//D5-A4
	GPIOA->CRL &= ~GPIO_CRL_MODE4;   //очистить разряды MODE
	GPIOA->CRL &= ~GPIO_CRL_CNF4;    //очистить разряды CNF
	GPIOA->CRL |=  GPIO_CRL_CNF4_0;  //дискретный вход, третье состояние
	//D6-A1
	GPIOA->CRL &= ~GPIO_CRL_MODE1;   //очистить разряды MODE
	GPIOA->CRL &= ~GPIO_CRL_CNF1;    //очистить разряды CNF
	GPIOA->CRL |=  GPIO_CRL_CNF1_0;  //дискретный вход, третье состояние	
	//D7-A2
	GPIOA->CRL &= ~GPIO_CRL_MODE2;   //очистить разряды MODE
	GPIOA->CRL &= ~GPIO_CRL_CNF2;    //очистить разряды CNF
	GPIOA->CRL |=  GPIO_CRL_CNF2_0;  //дискретный вход, третье состояние	
	...
	// Pulse length in us
	uint32_t del;
	del=LCD16X2_DELAY_ENABLE_PULSE;
	while(del--);
